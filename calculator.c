...
void CargarPila(void){
    /*Esta es la funcion para subir elementos a la pila: stack[0] => [7]*/
    for(int i = 7; i >= 0;i--){
        stack[i] = stack[i - 1];
    }
}
...
void DescargarPila(void){
     /* esta es la Funcion para bajar elementos de la pila stack[7] => stack[0]
     OJO SI SABEMOS que la visualizacion funciona al inverso */
     int i = 0;
     do{
        stack[i - 1] = stack[i];
        i++;
...
void addToStack(float num_to_add){
    //Agregacion de valores a la pila por medio de esta funcion/
    stack[7] = num_to_add;
    printf("\n\n");
}
...
int main(){
    ...
    float num, r= 0;
    char c;
    //este es el Ciclo para imprimir el menu principal/
    do{
        printMenu();
        while ((c = getchar()) != '\n' && c != EOF);
        if (scanf("%d", &aux) != 1 || aux > 5 || aux < 0){
            ...
            //Este es el menu con las operaciones para hacer
                printf("\n");
                printf("-------------------------------------\n");
